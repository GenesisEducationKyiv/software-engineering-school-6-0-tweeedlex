...
export async function buildGraph(config: Config, rootLogger: ILogger): Promise<AppGraph> {
  const prisma = createPrismaClient(
    config.databaseUrl,
    config.nodeEnv,
    rootLogger.child({ component: 'prisma' }),
  );
  ...
  const githubCache = new GitHubCache(
    ...
    githubLogger.child({ component: 'cache' }),
  );
  ...
  const notificationService = new NotificationService(
    ...
    config.baseUrl,
    metrics,
    rootLogger.child({ module: 'notifications' }),
  );
  ...
  const subscriptionService = new SubscriptionService(
    ...
    eventBus,
    validator,
    rootLogger.child({ module: 'subscriptions' }),
  );
  const scannerService = new ScannerService(
    ...
    eventBus,
    metrics,
    rootLogger.child({ module: 'scanner' }),
  );

  const notificationProducer = new BullMQProducer<NotificationJob>(
    NOTIFICATION_QUEUE,
    bullmq.getConnection(),
    rootLogger.child({ component: 'bullmq', queue: 'notifications' }),
  );
  ...
  const notificationWorker = buildNotificationWorker(
    workerFactory,
    notificationService,
    rootLogger.child({ module: 'notifications', component: 'worker' }),
  );
