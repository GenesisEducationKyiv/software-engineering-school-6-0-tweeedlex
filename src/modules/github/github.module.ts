...
import type { IGitHubService } from './github.types';
...
export type { GitHubRepo, GitHubRelease, IGitHubService } from './github.types';
  ...
  c.register(GITHUB_SERVICE, {
    useFactory: (dep) => {
      ...
      if (config.githubTransport === 'grpc') {
        return new GitHubGrpcClient({
          address: config.githubGrpcAddr,
          apiKey: config.apiKey,
          logger,
        });
      }
      return new GitHubHttpClient({
        baseUrl: config.githubServiceHttpUrl,
        apiKey: config.apiKey,
        logger,
      });
    },
