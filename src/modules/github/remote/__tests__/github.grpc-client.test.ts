...
function makeClient() {
  const grpcClient = new GitHubGrpcClient({ address: 'localhost:50062', apiKey: 'k', logger: mockLogger });
  const MockClient = GitHubServiceClient as unknown as jest.Mock;
  const mockInstance = MockClient.mock.results[
    MockClient.mock.results.length - 1
...
describe('GitHubGrpcClient', () => {
  it('verifyRepo maps proto response to GitHubRepo', async () => {
    ...
    const protoRepo = { id: 1, fullName: 'a/b', name: 'b', ownerLogin: 'a', description: '', htmlUrl: 'https://x', private: false };
    mock.verifyRepo.mockImplementation((_req: unknown, _meta: unknown, cb: Function) => cb(null, protoRepo));
  ...
  it('verifyRepo maps NOT_FOUND to NotFoundError', async () => {
    ...
    mock.verifyRepo.mockImplementation((_req: unknown, _meta: unknown, cb: Function) => cb(err, null));
  ...
  it('getLatestRelease returns null when found=false', async () => {
    const { grpc: client, mock } = makeClient();
    mock.getLatestRelease.mockImplementation((_req: unknown, _meta: unknown, cb: Function) => cb(null, { found: false }));
  ...
  it('getLatestRelease maps release fields', async () => {
    const { grpc: client, mock } = makeClient();
    const release = { id: 1, tagName: 'v1', name: 'v1', body: '', htmlUrl: 'https://x', publishedAt: '2024', draft: false, prerelease: false };
    mock.getLatestRelease.mockImplementation((_req: unknown, _meta: unknown, cb: Function) => cb(null, { found: true, release }));
    const result = await client.getLatestRelease('a', 'b');
